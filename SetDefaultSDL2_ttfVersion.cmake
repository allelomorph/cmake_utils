...
#   queries system package manager to get default version of SDL2_ttf library,
#     caching SDL2_TTF_DEFAULT_VERSION in <major>.<minor>.<patch> format
...
if(DEFINED CACHE{SDL2_TTF_DEFAULT_VERSION})
  return()
endif()
...
unset(_SDL2_TTF_DEFAULT_VERSION)

if(${PACKAGE_MANAGER_BINARY} MATCHES "/brew$")

  # MacOS: homebrew
  default_package_version("sdl2_ttf"          _SDL2_TTF_DEFAULT_VERSION)

elseif(${PACKAGE_MANAGER_BINARY} MATCHES "/aptitude$" OR
    ${PACKAGE_MANAGER_BINARY} MATCHES "/apt$")

  # Debian: aptitude > apt [> dpkg]
  default_package_version("libsdl2-ttf-2.0-0" _SDL2_TTF_DEFAULT_VERSION)
...
elseif(${PACKAGE_MANAGER_BINARY} MATCHES "/emerge")
  ...
  default_package_version("sdl2-ttf"          _SDL2_TTF_DEFAULT_VERSION)

elseif(${PACKAGE_MANAGER_BINARY} MATCHES "/zypper$")

  # openSUSE: zypper [> rpm]
  # note: passing "" to `zypper info` will list all packages
  default_package_version("libSDL2_ttf-2_0-0" _SDL2_TTF_DEFAULT_VERSION)

else()
  message(WARNING "`${PACKAGE_MANAGER_BINARY}` is unsupported package manager")
endif()

if(NOT ${_SDL2_TTF_DEFAULT_VERSION} MATCHES "-NOTFOUND$")
  upstream_ver_from_package_ver(
    "${_SDL2_TTF_DEFAULT_VERSION}" _SDL2_TTF_DEFAULT_VERSION)
...
set(SDL2_TTF_DEFAULT_VERSION "${_SDL2_TTF_DEFAULT_VERSION}" CACHE STRING
  "default SDL2_ttf package version in <major>.<minor>.<patch> format")
unset(_SDL2_TTF_DEFAULT_VERSION)
